...
distrobox enter ubuntu -- bash -c "
    ...
    sudo apt install -y shellcheck devscripts dput gnome-keyring git curl build-essential whiptail libdw-dev gcc git libncurses-dev curl gawk flex bison openssl libssl-dev dkms libelf-dev libudev-dev libpci-dev libiberty-dev autoconf make rustc bc rsync python-is-python3 perl gettext cpio pahole debhelper dwarves zstd \
